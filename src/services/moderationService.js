...
import { LokroBotError, ErrorTypes } from '../utils/errorHandler.js';
  ...
  static async banUser({
    ...
    try {
      ...
      if (targetMember) {
        ...
        if (!botCheck.valid) {
          throw new LokroBotError(botCheck.error, ErrorTypes.PERMISSION, botCheck.error);
        }

        const modCheck = this.validateHierarchy(moderator, targetMember, 'ban');
        if (!modCheck.valid) {
          throw new LokroBotError(modCheck.error, ErrorTypes.PERMISSION, modCheck.error);
        }
      } else {
